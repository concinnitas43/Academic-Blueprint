...
int remove_prereq(Subject* child, Subject* parent) {
    int parent_found = 0; // 부모 찾음 여부 플래그
    int child_found = 0;  // 자식 찾음 여부 플래그

    // 부모에서 자식 제거
    for (int i = 0; i < parent->child_count; i++) {
        if (parent->childs[i] == child) {
            parent_found = 1; // 부모 찾음
            for (int j = i; j < parent->child_count - 1; j++) {
                parent->childs[j] = parent->childs[j + 1]; // 자식 배열 이동
            }
            parent->child_count--; // 자식 수 감소
            parent->childs = (Subject**)realloc(parent->childs, parent->child_count * sizeof(Subject*)); // 자식 배열 재할당
            break; // 반복문 종료
        }
    }
    ...
    for (int i = 0; i < child->parent_count; i++) {
        if (child->parents[i] == parent) {
            ...
            for (int j = i; j < child->parent_count - 1; j++) {
                child->parents[j] = child->parents[j + 1]; // 부모 배열 이동
            }
            child->parent_count--; // 부모 수 감소
            child->parents = (Subject**)realloc(child->parents, child->parent_count * sizeof(Subject*)); // 부모 배열 재할당
            break; // 반복문 종료
        }
    }
    if (parent_found == 0) printf("Can't remove since no prerequisite relationship exist between %s and %s\n", parent->name, child->name); // 관계 없음 메시지 출력
    else printf("Successfully removed the prerequisite relationship between %s and %s\n", parent->name, child->name); // 성공 메시지 출력
    return (parent_found && child_found) ? 0 : -1; // 성공적으로 제거됨 여부 반환
}

// 큐 초기화 함수
Queue* create_queue(int capacity) {
    Queue* queue = (Queue*)malloc(sizeof(Queue)); // Queue에 대한 메모리 할당
    queue->subjects = (Subject**)malloc(capacity * sizeof(Subject*)); // Subject 배열에 대한 메모리 할당
    queue->front = 0; // 큐의 앞부분 인덱스 초기화
    queue->rear = 0;  // 큐의 뒷부분 인덱스 초기화
    queue->capacity = capacity; // 큐의 용량 설정
    return queue; // 초기화된 Queue 반환
}

// 큐 삽입 함수
void enqueue(Queue* queue, Subject* subject) {
    if (queue->rear == queue->capacity) { // 큐가 가득 찬 경우
        queue->capacity *= 2; // 용량을 두 배로 증가
        queue->subjects = (Subject**)realloc(queue->subjects, queue->capacity * sizeof(Subject*)); // Subject 배열 재할당
    }
    queue->subjects[queue->rear++] = subject; // Subject를 큐에 추가하고 rear 인덱스 증가
}

// 큐 삭제 함수
Subject* dequeue(Queue* queue) {
    if (queue->front == queue->rear) { // 큐가 비어있는 경우
        return NULL; // NULL 반환
    }
    return queue->subjects[queue->front++]; // 큐에서 Subject를 제거하고 front 인덱스 증가
}

// 큐가 비어있는지 확인하는 함수
int is_empty(Queue* queue) {
    return queue->front == queue->rear; // front와 rear가 같으면 큐가 비어있음
}
...
int check_cycle(Subject* subject) {
    ...
    int visited_size = 100; // 방문 배열 크기 설정
    Subject** visited = (Subject**)calloc(visited_size, sizeof(Subject*)); // 방문 배열 초기화

    while (!is_empty(queue)) { // 큐가 비어있지 않은 동안 반복
        ...
        // 방문하지 않은 노드라면 방문 표시
        for (int i = 0; i < visited_size; i++) {
            if (visited[i] == NULL) { // 방문 배열에 빈 공간이 있으면
        ...
        for (int i = 0; i < current->parent_count; i++) {
            enqueue(queue, current->parents[i]); // 부모 과목을 큐에 추가
        }
    }
